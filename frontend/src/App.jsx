...
function App() {
  ...
  const descargarPDF = () => {
    try {
      ...
      const tasaExitoPDF = totalEgresosPDF > 0 
        ? Math.round((registrosFiltrados.filter(r => r.tipo_evento === 'Egreso' && (r.destino === 'Liberado' || r.destino === 'Liberación')).reduce((acc, curr) => acc + (parseInt(curr.numero_ejemplar) || 0), 0) / totalEgresosPDF) * 100) 
        : 0;
      ...
      doc.text(`Total de Ingresos: ${totalIngresosPDF} ejemplares`, 40, currentY);
      doc.text(`Total de Egresos: ${totalEgresosPDF} ejemplares`, 250, currentY);
      doc.text(`Pacientes Activos (Neto): ${Math.max(0, totalIngresosPDF - totalEgresosPDF)}`, 450, currentY);
      
      currentY += 15;
      doc.text(`Tasa de Liberación (Éxito Clínico): ${tasaExitoPDF}%`, 40, currentY);
