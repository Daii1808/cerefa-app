...
class RegistroEvento(db.Model):
    ...
    numero_ficha = db.Column(db.String(50), nullable=False)
    especie_id = db.Column(db.Integer, db.ForeignKey('catalogo_fauna.id'), nullable=False)
    
    numero_acta_movimiento = db.Column(db.String(50), nullable=True)
    numero_ejemplar = db.Column(db.Integer, default=1)
    tipo_evento = db.Column(db.String(50))
    categoria_evento = db.Column(db.String(100), nullable=True)
    destino = db.Column(db.String(100), nullable=True)
    observacion = db.Column(db.Text, nullable=True)

    saldo_anterior = db.Column(db.Integer, nullable=False)
    saldo_actual = db.Column(db.Integer, nullable=False)
