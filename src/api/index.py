...
# 1. Configuración de rutas para que los imports funcionen en Vercel
...
app = Flask(__name__)
application = app  # Vercel busca "application" como entrypoint
...
def home():
        ...
        <div class="bg-zinc-900 p-6 rounded-xl border border-zinc-800 overflow-x-auto">
            <table class="w-full text-sm text-left">
                <thead class="text-zinc-500 uppercase border-b border-zinc-800">
                    <tr><th class="py-2">Ficha</th><th class="py-2">Especie</th><th class="py-2">Obs</th></tr>
                </thead>
                <tbody class="divide-y divide-zinc-800">
                    {% for r in lista %}
                    <tr><td class="py-3 font-mono">{{ r.numero_ficha }}</td><td class="py-3">{{ r.especie.nombre_comun if r.especie else 'N/A' }}</td><td class="py-3">{{ r.observacion or '-' }}</td></tr>
                    {% endfor %}
                </tbody>
            </table>
        </div>
    ...
    """
    return render_template_string(html, lista=lista, error_msg=error_msg)
...
def web_registrar():
    try:
        RegistroEvento.registrar_con_saldo(
            ...
            tipo_evento="Ingreso",  # Valor fijo según tu solicitud
            numero_ejemplar=1  # Valor fijo según tu solicitud
        )
    except Exception as e:
        print(f"Error registrando: {e}")
        # Retornamos error al frontend para que se muestre bonito
